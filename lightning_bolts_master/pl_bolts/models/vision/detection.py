...
from dice_loss import Class_Wise_TIL_Detection_FROC
from pytorch_lightning.callbacks import LearningRateMonitor, ModelCheckpoint
from u_net_resnet_18_encoder import UNetWithResnet18Encoder
...
class SemSegment(LightningModule):
    def __init__(
        ...
        self.net = UNetWithResnet18Encoder(n_classes = 2)
    ...
    def training_step(self, batch, batch_nb):
        ...
        #self.log('train_sens', sens, on_step=False, on_epoch=True)
        #self.log('train_fps', ftp, on_step=False, on_epoch=True)
        #self.log('train_dice_loss', dice_total_loss, on_step=False, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (batch_idx == 0) and self.after_training:
            for i in range(len(argmax_pred)):
                ...
                argmax_mask0 = Lambda(lambda x: x[i, :, :])(np_mask)
                bmp1 = Image.fromarray(np.array(argmax_pred0, dtype = 'uint8'))
                bmp2 = Image.fromarray(np.array(argmax_mask0, dtype='uint8'))
                dst = 'pred_image' + str(a) + '.png'
                pallette = [250,0,250, 0,250,0]
                bmp1.putpalette(pallette)
        ...
        #self.log('val_sens', sens, on_step=False, on_epoch=True)
        #self.log('val_fps', ftp, on_step=False, on_epoch=True)
        #self.log('val_dice_loss', dice_total_loss, on_step=False, on_epoch=True)
        self.log('val_bce_loss', bce_loss, on_step=False, on_epoch=True)
        self.log('val_froc', froc_score, on_step=False, on_epoch=True)
...
def cli_main():
    ...
    args.__dict__["batch_size"] = 15
    args.__dict__["precision"] = 32
    args.__dict__["logger"] = tb_loggers
    args.__dict__["max_epochs"] = 25
    args.__dict__["num_workers"] = 40
    args.__dict__["callbacks"] = [ModelCheckpoint(save_top_k=3, save_last=True, monitor="val_loss"),
