...
def resnet18(pretrained: bool = True, progress: bool = True, **kwargs):
    ...
    #state = torchvision.models.resnet.resnet34(pretrained=True)
    resnet18 = _resnet18("resnet18", BasicBlock, [2, 2, 2, 2], pretrained, progress, **kwargs)
    #state = torchvision.models.resnet.resnet18(pretrained=True)
    ...
    encoder = load_encoder_weights(resnet18, state_dict)
    '''
...
def resnet34(pretrained: bool = True, progress: bool = True, **kwargs):
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    Args:
        ...
        progress: If True, displays a progress bar of the download to stderr
    """
    resnet34 = _resnet34("resnet34", BasicBlock, [3, 4, 6, 3], pretrained, progress, **kwargs)
...
def _resnet18(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(block, layers, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(MODEL_URLS[arch], progress=progress)

        model = load_encoder_weights(model, state_dict)

        #model.load_state_dict(state_dict)
    return model


def _resnet34(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(block, layers, **kwargs)
    MODEL_PATH = "/home/skim/my-project/lib/python3.6/site-packages/pl_bolts/models/vision/deeplabv3/pretrained_models/resnet/resnet34-333f7ec4.pth"
    if pretrained:
        resnet = torchvision.models.resnet.resnet34(pretrained=False)
        # pretrained 모델의 저장된 파라미터 불러오기
...
def _resnet50(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(block, layers, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(MODEL_URLS[arch], progress=progress)
