...
class CervixDataset(Dataset):
    ...
    #IMAGE_PATH = os.path.join("test", "imgs")
    #MASK_PATH = os.path.join("test", "masks")
    ...
    def __init__(
        ...
        #self.img_list = self.folder_list_loader(self.folder_list)
        #self.mask_list = self.get_filenames(self.mask_path)

        self.img_label_list = self.get_filenames(self.file_list )
        #self.mask_list = self.get_filenames(self.mask_path)
    ...
    def __getitem__(self, idx):
        ...
        img_path = self.img_label_list[idx].split()[0]
        label = self.img_label_list[idx].split()[1]
    ...
    # def folder_list_loader(self, path, sub_path='img'):
    #     with open(path, 'r') as f:
    #         lines = f.readlines()
    #
    #         all_X_list = []
    #
    #         for line in lines:
    #             l_folder_path, label = line.split()[0], line.split()[1] 	# folder path
    #             img_folder_path = os.path.join(l_folder_path, sub_path)	# img folder path
    #
